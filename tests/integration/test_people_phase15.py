...
def test_queue_binds_exact_fingerprint_and_priority_is_deterministic(
    tmp_path: Path,
) -> None:
    with database_session(tmp_path / "queue.sqlite3") as connection:
        ...
        first = list_work_queue(
            connection, WorkQueueFilters(person_id=person_id, reference_time=REFERENCE)
        )
        assert len(first) == 1
        row = first[0]
        ...
        create_task(
            ...
            actor="alex",
            owner="alex",
            due_at="2026-08-12T11:00:00Z",
        )
        current = list_work_queue(
            connection, WorkQueueFilters(person_id=person_id, reference_time=REFERENCE)
        )[0]
...
def test_task_states_filters_owner_and_export_are_deterministic(tmp_path: Path) -> None:
    with database_session(tmp_path / "filters.sqlite3") as connection:
        ...
        assert (
            list_work_queue(
                connection,
                WorkQueueFilters(has_remediation=True, reference_time=REFERENCE),
            )[0]["person_id"]
            == person_id
        )
        assert (
            list_work_queue(
                connection, WorkQueueFilters(owner="alex", reference_time=REFERENCE)
            )[0]["person_id"]
            == person_id
        )
        assert (
        ...
        blocked = list_work_queue(
            connection, WorkQueueFilters(blocked_only=True, reference_time=REFERENCE)
        )[0]
        ...
        assert (first / "person_work_queue.csv").read_bytes() == (
            second / "person_work_queue.csv"
        ).read_bytes()
        assert (first / "person_work_queue_owners.csv").read_bytes() == (
            second / "person_work_queue_owners.csv"
        ).read_bytes()
