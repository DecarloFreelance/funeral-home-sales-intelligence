...
def _open_anomaly(connection, person_id: int) -> dict[str, object]:
    ...
    connection.commit()
    return triage_people(connection, TriageFilters(person_id=person_id))[0][
        "anomalies"
    ][0]
...
def test_fingerprint_is_stable_and_structured(tmp_path: Path) -> None:
    ...
    assert fingerprint_anomaly(
        1, {**same, "values": ["a", "c"]}
    ) != fingerprint_anomaly(1, same)
    assert fingerprint_anomaly(1, same) == fingerprint_anomaly(
        1, {**same, "timestamp": "different"}
    )
...
def test_disposition_lifecycle_and_exact_instance_validation(tmp_path: Path) -> None:
    with database_session(tmp_path / "lifecycle.sqlite3") as connection:
        ...
        result = decide_disposition(
            ...
            status=DispositionStatus.DISMISSED,
            actor="alex",
            note="accepted legacy gap",
        )
        ...
        reopened = decide_disposition(
            connection,
            person_id=person_id,
            anomaly_code=anomaly["code"],
            fingerprint=fingerprint,
            status=DispositionStatus.REOPENED,
            actor="sam",
        )
        assert reopened.status is DispositionStatus.REOPENED
        acknowledged = decide_disposition(
            connection,
            person_id=person_id,
            anomaly_code=anomaly["code"],
            fingerprint=fingerprint,
            status=DispositionStatus.ACKNOWLEDGED,
            actor="sam",
        )
        ...
        assert (
            triage_people(connection, TriageFilters(disposition_status="acknowledged"))[
                0
            ]["person_id"]
            == person_id
        )
...
def test_sync_stales_changed_fingerprint_and_is_idempotent(tmp_path: Path) -> None:
    with database_session(tmp_path / "stale.sqlite3") as connection:
        ...
        connection.execute(
            "INSERT INTO person_evidence (person_id, observation_id, review_decision) SELECT ?, observation_id, 'accepted' FROM person_observation_review_queue LIMIT 1",
            (person_id,),
        )
        connection.commit()
        ...
        assert (
            show_disposition(connection, result.disposition_id)["current_anomaly"]
            is None
        )
...
def test_history_is_immutable_and_exports_are_deterministic(tmp_path: Path) -> None:
    with database_session(tmp_path / "export.sqlite3") as connection:
        ...
        history_id = int(
            connection.execute(
                "SELECT id FROM person_anomaly_disposition_history WHERE disposition_id = ?",
                (result.disposition_id,),
            ).fetchone()[0]
        )
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "UPDATE person_anomaly_disposition_history SET note = 'tamper' WHERE id = ?",
                (history_id,),
            )
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "DELETE FROM person_anomaly_disposition_history WHERE id = ?",
                (history_id,),
            )
        first, second = tmp_path / "one", tmp_path / "two"
        export_people_csv(connection, first)
        export_people_csv(connection, second)
        assert (first / "person_anomaly_dispositions.csv").read_bytes() == (
            second / "person_anomaly_dispositions.csv"
        ).read_bytes()
        assert (first / "person_anomaly_disposition_history.csv").read_bytes() == (
            second / "person_anomaly_disposition_history.csv"
        ).read_bytes()
