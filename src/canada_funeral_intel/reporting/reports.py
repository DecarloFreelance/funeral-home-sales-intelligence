...
def _metric(
    metric_id: str, numerator: int, denominator: int, *, excluded: int = 0
) -> dict[str, Any]:
    return {
...
def coverage_report(
    ...
    *,
    ...
    entity_rows = connection.execute(
        f"SELECT COUNT(*) AS count, SUM(entity_type='organization') AS organizations, SUM(entity_type='branch') AS branches FROM entities e WHERE {entity_filter}"
    ).fetchone()
    metrics = [
        ...
        _metric(
            "entities_with_people_observation", row["observation_count"], denominator
        ),
    ...
    return {
        ...
        "entity_counts": {
            "total": int(entity_rows["count"]),
            "organizations": int(entity_rows["organizations"] or 0),
...
def quality_report(
    ...
    *,
    ...
    distributions = {
        label: sum(row["readiness"] == label for row in results)
        for label in ("insufficient_evidence", "low", "medium", "high")
    }
    by_type: list[dict[str, Any]] = []
    for subject_type in SUBJECT_TYPES:
        group = [row for row in results if row["subject_type"] == subject_type]
        scores = [
            row["overall_score"] for row in group if row["overall_score"] is not None
        ]
        by_type.append(
            {
                ...
                "conflict_count": sum(
                    "conflicting_values" in row["warnings"] for row in group
                ),
                "incomplete_count": sum(
                    "missing_provenance" in row["reasons"] for row in group
                ),
            }
        )
    return {
        ...
        "reference_time": _reference(reference_time),
        "include_historical": include_historical,
        "readiness_distribution": distributions,
        "subjects": by_type,
    }
    ...
    *,
    ...
    rows = [
        ...
        for row in connection.execute(
            f"SELECT * FROM {source} ORDER BY entity_id, website_id, website_page_id, fact_key, normalized_value, id"
        )
    ]
    groups: dict[tuple[Any, ...], list[dict[str, Any]]] = {}
    for row in rows:
        groups.setdefault(
            (row["entity_id"], row["fact_key"], row["scope"], row["scope_entity_id"]),
            [],
        ).append(row)
    states = {
        state: 0 for state in ("observed", "repeated", "conflict", "ambiguous_scope")
    }
...
def people_report(
    ...
    *,
    ...
    return {
        ...
        "contact_counts": {
            "with_email": int(row["email_count"] or 0),
            "with_phone": int(row["phone_count"] or 0),
        },
        "review_counts": {
            key: int(row[key] or 0) for key in ("accepted", "rejected", "deferred")
        },
        "remediation_counts": {
            key: int(row[key] or 0) for key in ("active", "blocked", "stale", "overdue")
        },
    }
