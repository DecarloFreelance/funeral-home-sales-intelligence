...
def _input(path: Path, *, changed: bool = False) -> Path:
    ...
    path.write_text(json.dumps([{"id": "a", "name": value, "city": "Calgary", "province": "AB", "phone": "403-555-0100", "postal_code": "T2P1J9"}]), encoding="utf-8")
    return path


def test_pipeline_runs_end_to_end_and_identical_rerun_is_idempotent(tmp_path: Path) -> None:
    ...
    with database_session(database_path) as connection:
        ...
        first_counts = {table: connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] for table in ("source_records", "normalized_values", "entities", "entity_source_records", "pipeline_run_stages")}
        second = create_run(connection, spec)
        second_counts = {table: connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] for table in ("source_records", "normalized_values", "entities", "entity_source_records")}

    assert first["status"] == "completed"
    assert all(stage["status"] == "completed" for stage in first["stages"])
    assert second["status"] == "completed"
    assert second_counts == {key: value for key, value in first_counts.items() if key != "pipeline_run_stages"}
...
def test_failed_stage_is_persisted_and_resume_rejects_changed_input(tmp_path: Path) -> None:
    database_path, dataset_id = _database(tmp_path / "failure.sqlite3")
    input_path = _input(tmp_path / "records.json")
    input_path.write_text("not-json", encoding="utf-8")
    with database_session(database_path) as connection:
        with pytest.raises(PipelineError, match="Invalid JSON"):
            create_run(connection, PipelineInput(dataset_id, input_path, ImportFormat.JSON, "id", through_stage="import"))
        ...
        with pytest.raises(sqlite3.IntegrityError, match="immutable"):
            connection.execute("UPDATE pipeline_run_errors SET error_message = 'changed' WHERE id = ?", (error["id"],))
        with pytest.raises(sqlite3.IntegrityError, match="immutable"):
            connection.execute("DELETE FROM pipeline_run_errors WHERE id = ?", (error["id"],))
        input_path.write_text(json.dumps([{"id": "a", "name": "Alpha"}]), encoding="utf-8")
...
def test_resume_claim_prevents_running_run_reuse(tmp_path: Path) -> None:
    ...
    input_path = _input(tmp_path / "records.json")
    with database_session(database_path) as connection:
        result = create_run(connection, PipelineInput(dataset_id, input_path, ImportFormat.JSON, "id", through_stage="import"))
        run_id = int(result["id"])
        connection.execute("UPDATE pipeline_runs SET status = 'running' WHERE id = ?", (run_id,))
        connection.commit()
...
def test_resume_skips_completed_stages(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with database_session(database_path) as connection:
        with pytest.raises(PipelineError, match="fixture review failure"):
            create_run(connection, PipelineInput(dataset_id, input_path, ImportFormat.JSON, "id"))
        run_id = int(connection.execute("SELECT id FROM pipeline_runs").fetchone()[0])
        completed_before = connection.execute("SELECT stage_name FROM pipeline_run_stages WHERE pipeline_run_id=? AND status='completed' ORDER BY ordinal", (run_id,)).fetchall()
        assert [row["stage_name"] for row in completed_before] == ["import", "normalize", "deterministic_match", "fuzzy_match"]
