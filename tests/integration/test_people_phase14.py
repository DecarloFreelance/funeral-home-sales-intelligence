...
def test_task_lifecycle_and_exact_fingerprint_binding(tmp_path: Path) -> None:
    with database_session(tmp_path / "tasks.sqlite3") as connection:
        ...
        assert (
            ...
            is RemediationStatus.IN_PROGRESS
        )
        assert (
            update_task(
                connection,
        ...
        assert connection.execute(
            "SELECT owner, due_at FROM person_anomaly_remediation_tasks WHERE id = ?",
            (task.task_id,),
        ...
        with pytest.raises(PersonResolutionError):
            update_task(
                ...
                task_id=task.task_id,
                actor="alex",
                status=RemediationStatus.STALE,
            )
...
def test_task_summary_filters_overdue_and_audit_are_read_only(tmp_path: Path) -> None:
    with database_session(tmp_path / "summary.sqlite3") as connection:
        ...
        before = {
            ...
            for table in ("people", "person_evidence", "person_anomaly_dispositions")
        }
        task = create_task(
            connection,
            person_id=person_id,
        ...
        assert (
            triage_people(connection, TriageFilters(has_remediation=True))[0][
                "person_id"
            ]
            == person_id
        )
        assert (
            triage_people(
                connection, TriageFilters(no_remediation=True, anomaly=anomaly["code"])
            )
            == []
        )
        assert (
            triage_people(connection, TriageFilters(overdue_remediation=True))[0][
                "person_id"
            ]
            == person_id
        )
        assert (
            list_tasks(connection, overdue_only=True, now="2021-01-01T00:00:00Z")[0][
                "id"
            ]
            == task.task_id
        )
...
def test_sync_stales_task_without_creating_new_task(tmp_path: Path) -> None:
    with database_session(tmp_path / "sync.sqlite3") as connection:
        ...
        assert (
            triage_people(connection, TriageFilters(person_id=person_id))[0][
                "anomalies"
            ]
            == []
        )
...
def test_history_is_immutable_and_exports_repeatably(tmp_path: Path) -> None:
    with database_session(tmp_path / "export.sqlite3") as connection:
        ...
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "UPDATE person_anomaly_remediation_task_history SET note = 'tamper' WHERE id = ?",
                (history_id,),
            )
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "DELETE FROM person_anomaly_remediation_task_history WHERE id = ?",
                (history_id,),
            )
