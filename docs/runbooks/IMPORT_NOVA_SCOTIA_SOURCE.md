...
The exact external-ID and field names must be confirmed from the downloaded
file before execution. Do not substitute a guessed identifier. Unknown fields
remain in `source_records.raw_payload`; recognized `website`, `website_url`,
`url`, and explicit website aliases become normalized website signals.
