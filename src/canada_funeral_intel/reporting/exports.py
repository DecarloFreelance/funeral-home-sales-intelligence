...
def export_reports(connection, output: Path, *, include_historical: bool = False, reference_time=None) -> dict[str, Any]:
    ...
    for name in sorted(reports):
        ...
        paths.append(path)
    coverage_fields = ("definition_id", "numerator", "denominator", "excluded", "percentage")
    ...
    manifest = {"report_version": REPORT_VERSION, "reference_time": reports["coverage"]["reference_time"], "include_historical": include_historical, "files": []}
    for path in sorted(paths, key=lambda value: value.name):
        data = path.read_bytes()
        manifest["files"].append({"name": path.name, "bytes": len(data), "sha256": content_hash(data.decode("utf-8"))})
    manifest_path = output / "report_manifest.json"
    manifest_path.write_bytes(stable_json(manifest))
    paths.append(manifest_path)
    return {"format": "json-csv", "output": str(output), "files": [path.name for path in sorted(paths, key=lambda value: value.name)], "manifest": manifest}
