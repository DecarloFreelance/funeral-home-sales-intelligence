...
def _observation(
    ...
    *,
    ...
    email: str = "",
    phone: str = "",
    role: str = "Funeral Director",
    number: int = 1,
) -> int:
    digest = hashlib.sha256(
        f"{entity_id}-{number}-{name}-{email}-{phone}".encode()
    ).hexdigest()
    cursor = connection.execute(
        ...
        (
            ...
            None,
            f"{name} — {role}",
            f"https://example{entity_id}.ca/team",
            digest,
        ),
    )
...
def _database(connection):
    ...
    for name in ("Home A", "Home B"):
        cursor = connection.execute(
            "INSERT INTO entities (entity_type, canonical_name) VALUES ('organization', ?)",
            (name,),
        )
    ...
    for entity_id in entities:
        website = connection.execute(
            "INSERT INTO websites (entity_id, url, normalized_url, domain, discovery_method, status) VALUES (?, ?, ?, ?, 'test', 'review')",
            (
                entity_id,
                f"https://example{entity_id}.ca/",
                f"https://example{entity_id}.ca/",
                f"example{entity_id}.ca",
            ),
        )
        website_id = int(website.lastrowid)
        page = connection.execute(
            "INSERT INTO website_pages (website_id, url, normalized_url, path, page_kind, status_code, content_type) VALUES (?, ?, ?, '/team', 'team', 200, 'text/html')",
            (
                website_id,
                f"https://example{entity_id}.ca/team",
                f"https://example{entity_id}.ca/team",
    ...
    connection.execute(
        "INSERT INTO website_review_queue (website_id) VALUES (?)", (websites[0],)
    )
    return entities, websites, pages


def test_person_review_resolution_is_conservative_and_provenance_rich(
    tmp_path: Path,
) -> None:
    with database_session(tmp_path / "people.sqlite3") as connection:
        ...
        assert run_people_review_list(
            connection, PersonReviewStatus.PENDING
        ) == run_people_review_list(connection, PersonReviewStatus.PENDING)
        assert [row["observation_id"] for row in queues] == [
            first,
            repeated,
            other_branch,
            conflicting,
        ...
        apply_person_review_decision(
            connection,
            queue_id=int(queues[-1]["queue_id"]),
        ...
        source = connection.execute(
            "SELECT * FROM website_page_person_observations WHERE id = ?", (first,)
        ).fetchone()
        evidence = connection.execute(
            "SELECT observation_id FROM person_evidence WHERE observation_id = ?",
            (first,),
        ).fetchone()
