...
class TriageFilters:
    ...
    def validate(self) -> None:
        ...
        if self.traceability is not None and self.traceability not in {"traceable", "incomplete", "orphaned"}:
...
def _supporting_ids(audit: dict[str, object], anomaly: dict[str, object]) -> dict[str, list[int]]:
    ...
    if anomaly.get("observation_ids"):
        ...
        observations = [row for row in observations if int(row["observation_id"]) in selected]
    if anomaly.get("affiliation_id") is not None:
        selected = int(anomaly["affiliation_id"])
        observations = [row for row in observations if any(int(item["affiliation_id"]) == selected and int(item["source_observation_id"]) == int(row["observation_id"]) for item in audit["affiliations"] + audit["historical_affiliations"])]
    if anomaly.get("contact_id") is not None:
        selected = int(anomaly["contact_id"])
        observations = [row for row in observations if any(int(item["contact_id"]) == selected and int(item["source_observation_id"]) == int(row["observation_id"]) for item in audit["contact_points"] + audit["historical_contact_points"])]
    if anomaly.get("merge_id") is not None:
        merge = int(anomaly["merge_id"])
        observations = [row for row in observations if any(int(item.get("merge_id", -1)) == merge for item in audit["historical_evidence"])]
    contact_ids = [anomaly.get("contact_id")]
    if str(anomaly.get("code", "")).startswith("conflicting_active_"):
        contact_type = "email" if anomaly["code"] == "conflicting_active_emails" else "phone"
        contact_ids.extend(row["contact_id"] for row in audit["contact_points"] if row["active"] == 1 and row["contact_type"] == contact_type)
    return {
...
def _anomaly_matches(anomaly: dict[str, object], *, code: str | None, severity: TriageSeverity | None) -> bool:
    if code is not None and anomaly["code"] != code:
        return False
    return severity is None or SEVERITY_BY_CODE.get(str(anomaly["code"]), TriageSeverity.LOW) is severity


def _record_matches(record: dict[str, object], filters: TriageFilters) -> bool:
    audit = record["_audit"]
    if (filters.anomaly is not None or filters.severity is not None) and not any(_anomaly_matches(item, code=filters.anomaly, severity=filters.severity) for item in audit["anomalies"]):
        return False
    if filters.traceability is not None and audit["traceability"]["status"] != filters.traceability:
        return False
    ...
    if filters.branch_id is not None and filters.branch_id not in record["branch_ids"]:
        return False
    if filters.website_id is not None and filters.website_id not in record["website_ids"]:
        return False
    if filters.page_id is not None and filters.page_id not in record["page_ids"]:
        return False
    if filters.review_status is not None and filters.review_status not in record["review_statuses"]:
        return False
...
def _build_record(connection: sqlite3.Connection, audit: dict[str, object]) -> dict[str, object]:
    ...
    source_observation_ids = {int(row["source_observation_id"]) for row in current_affiliations + current_contacts}
    if source_observation_ids:
        ...
        observation_rows.extend(dict(row) for row in provenance if int(row["observation_id"]) not in {int(item["observation_id"]) for item in observation_rows})
    entity_ids = _ids([row["entity_id"] for row in observation_rows] + [row["entity_id"] for row in current_affiliations])
    branch_ids = _ids([row["entity_id"] for row in current_affiliations if row.get("entity_type") == "branch"])
    website_ids = _ids([row["website_id"] for row in observation_rows])
    page_ids = _ids([row.get("website_page_id", row.get("page_id")) for row in observation_rows])
    anomalies = []
    ...
    return {
        ...
        "observation_count": len({int(row["observation_id"]) for row in observation_rows}),
        "accepted_review_count": sum(1 for row in audit["reviews"] if row["status"] == "accepted"),
        "rejected_review_count": sum(1 for row in audit["reviews"] if row["status"] == "rejected"),
        "deferred_review_count": sum(1 for row in audit["reviews"] if row["status"] == "deferred"),
        "pending_review_count": sum(1 for row in audit["reviews"] if row["status"] == "pending"),
        ...
        "rollback_count": sum(1 for row in audit["merge_history"] if row["state"] == "rolled_back"),
        "entity_ids": entity_ids,
        "branch_ids": branch_ids,
        "website_ids": website_ids,
        "page_ids": page_ids,
        "review_statuses": review_statuses,
        "emails": sorted({str(row["normalized_value"]) for row in current_contacts if row["contact_type"] == "email" and row["normalized_value"]}),
        "phones": sorted({str(row["normalized_value"]) for row in current_contacts if row["contact_type"] == "phone" and row["normalized_value"]}),
    }


def triage_people(connection: sqlite3.Connection, filters: TriageFilters | None = None) -> list[dict[str, object]]:
